...
const iconMap: Record<string, any> = { Heart, Hash, Users, Camera, Scissors };
...
const Differentials = () => {
  const { getValue } = useSiteContent("differentials");

  const sectionLabel = getValue("differentials", "section_label", "Por que nos escolher");
  const sectionTitle = getValue("differentials", "section_title", "Muito além do registro");
  const closingText = getValue("differentials", "closing_text", "A Racun Weddings é para quem valoriza histórias reais, emoção verdadeira e um olhar cinematográfico. Trabalhamos com número limitado de casamentos por ano para garantir atenção total a cada história.");
  const items = getValue("differentials", "items", defaultDifferentials);

  const resolvedItems = items.map((item: any, i: number) => ({
    ...item,
    IconComponent: iconMap[item.icon] ?? Object.values(iconMap)[i % Object.values(iconMap).length],
  }));
      ...
      <section className="py-20 md:py-28 bg-background">
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
          <AnimatedSection className="text-center mb-12 md:mb-16">
            ...
            <h2 className="font-heading text-3xl md:text-5xl font-light text-foreground">{sectionTitle}</h2>
            <div className="w-12 h-[2px] bg-primary mx-auto mt-5" />
          </AnimatedSection>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mb-6">
            {resolvedItems.slice(0, 3).map((item: any, i: number) => (
              <AnimatedSection key={item.title + i} delay={i * 0.1}>
                <div className="bg-accent/50 border border-border rounded-lg p-5 md:p-6 text-center h-full hover:shadow-md transition-shadow">
                  <div className="mx-auto w-11 h-11 flex items-center justify-center rounded-full bg-primary/10 mb-4">
                    {item.IconComponent && <item.IconComponent size={20} className="text-primary" strokeWidth={1.5} />}
                  </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-6 max-w-2xl mx-auto">
            {resolvedItems.slice(3).map((item: any, i: number) => (
              <AnimatedSection key={item.title + i} delay={(i + 3) * 0.1}>
                <div className="bg-accent/50 border border-border rounded-lg p-5 md:p-6 text-center h-full hover:shadow-md transition-shadow">
                  <div className="mx-auto w-11 h-11 flex items-center justify-center rounded-full bg-primary/10 mb-4">
                    {item.IconComponent && <item.IconComponent size={20} className="text-primary" strokeWidth={1.5} />}
                  </div>
      ...
      <section className="py-20 md:py-28 bg-secondary/30">
        <div className="container mx-auto px-4 md:px-6 max-w-5xl">
          <AnimatedSection className="text-center mb-12 md:mb-16">
            ...
            <h2 className="font-heading text-3xl md:text-5xl font-light text-foreground">Cada casamento é único</h2>
